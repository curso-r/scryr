...
#'   return the most up-to-date recognizable version of the card.
#' * Unique Artwork: A JSON file of Scryfall card objects that together contain
#'   all unique artworks. The chosen cards promote the best image scans.
#' * Default Cards: A JSON file containing every card object on Scryfall in
#'   English or the printed language if the card is only available in one
#'   language.
#' * All Cards: A JSON file containing every card object on Scryfall in every
#'   language.
